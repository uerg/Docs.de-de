...
Diese beiden Aufgaben – die Siteübersicht definieren und implementieren eine Navigation Benutzeroberfläche basierend auf der Siteübersicht – sind ganz einfach Dank der Siteübersicht-Framework und Navigation Websteuerelementen in ASP.NET Version 2.0 hinzugefügt. Im Siteübersicht können Entwickler auf eine Siteübersicht definieren und dann über eine programmgesteuerte-API auf Sie zugreifen (die [ `SiteMap` Klasse](https://msdn.microsoft.com/library/system.web.sitemap.aspx)). Die integrierte Navigation Websteuerelemente enthalten eine [Menu-Steuerelement](https://msdn.microsoft.com/library/bz09dy46.aspx), die [TreeView-Steuerelement](https://msdn.microsoft.com/library/3eafky27.aspx), und die [SiteMapPath-Steuerelement](https://msdn.microsoft.com/library/3eafky27.aspx).
...
Andere Verhalten-bezogene Eigenschaften Hinweis gehören `AutoGeneratePassword` und `MailDefinition`. Wenn die [ `AutoGeneratePassword` Eigenschaft](https://msdn.microsoft.com/library/system.web.ui.webcontrols.createuserwizard.autogeneratepassword.aspx) auf "true" festgelegt ist die `CreateUserWizardStep` wird nicht angezeigt, die Textfelder "Password" und "Kennwort bestätigen" ein; stattdessen das Kennwort des neu erstellten Benutzers wird automatisch generiert mithilfe der `Membership` Klasse [ `GeneratePassword` Methode](https://msdn.microsoft.com/library/system.web.security.membership.generatepassword.aspx). Die `GeneratePassword` -Methode erstellt ein Kennwort mit einer angegebenen Länge und mit einer ausreichenden Anzahl von nicht-alphanumerische Zeichen, das Kennwort Stärke Anforderungen zu erfüllen.
