...
Andere Verhalten-bezogene Eigenschaften Hinweis gehören `AutoGeneratePassword` und `MailDefinition`. Wenn die [ `AutoGeneratePassword` Eigenschaft](https://msdn.microsoft.com/en-us/library/system.web.ui.webcontrols.createuserwizard.autogeneratepassword.aspx) auf "true" festgelegt ist die `CreateUserWizardStep` wird nicht angezeigt, die Textfelder Kennwort und Kennwort bestätigen ein; stattdessen das Kennwort des neu erstellten Benutzers wird automatisch generiert mithilfe der `Membership` Klasse [ `GeneratePassword` Methode](https://msdn.microsoft.com/en-us/library/system.web.security.membership.generatepassword.aspx). Die `GeneratePassword` -Methode erstellt ein Kennwort mit einer angegebenen Länge und mit einer ausreichenden Anzahl von nicht-alphanumerische Zeichen, das Kennwort Stärke Anforderungen zu erfüllen.
