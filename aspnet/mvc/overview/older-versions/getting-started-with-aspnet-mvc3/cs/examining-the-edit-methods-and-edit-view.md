...
Die Lösung besteht darin, eine Überladung der `BeginForm` , der angibt, dass die POST-Anforderung die Suchinformationen, mit der URL hinzufügen sollte und, ist werden sollte weitergeleitet, auf die HttpGet-Version von der `SearchIndex` Methode. Ersetzen Sie die vorhandene parameterlosen `BeginForm` -Methode durch Folgendes:
