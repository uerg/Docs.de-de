...
ASP.NET oder Active Server Pages ist ASP.NET MVC nicht alles enthalten sein, die direkt auf eine Seite entspricht. In einer ASP.NET MVC-Anwendung besteht keine Seite auf dem Datenträger, der den Pfad in der URL entspricht, die Sie in der Adressleiste des Browsers eingeben. Am ehesten auf eine Seite in einer ASP.NET MVC-Anwendung ist etwas bezeichnet eine *Ansicht*.

ASP.NET MVC Anwendung, eingehende Browseranforderungen Controlleraktionen zugeordnet sind. Eine Controlleraktion könnte eine Sicht zurückgeben. Eine Controlleraktion führen jedoch möglicherweise aus einen anderen Typ der Aktion, z. B. umleiten Sie zu einem anderen Controlleraktion.
...
Verwenden Sie das Skript-Trennzeichen &lt;"und" %&gt; um Anfang und Ende eines Skripts zu markieren. Dieses Skript wird in c# geschrieben. Es zeigt das aktuelle Datum und die Uhrzeit an, durch Aufrufen der Methode Response.Write() für das Rendern von Inhalt an den Browser. Die Skript-Trennzeichen &lt;"und" %&gt; können verwendet werden, um eine oder mehrere Anweisungen ausführen.
...
**Abbildung 01**: ein Anmeldename-Standardformat ([klicken Sie hier, um das Bild in voller Größe angezeigt](asp-net-mvc-views-overview-cs/_static/image2.png))
...
**6 – ProductController.cs auflisten**
