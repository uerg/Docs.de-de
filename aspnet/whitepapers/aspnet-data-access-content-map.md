...
- [ASP.NET-Daten Zugriffsoptionen](https://msdn.microsoft.com/en-us/library/ms178359.aspx) (MSDN). Einführung in die Datenzugriffsoptionen für relationale Datenbanken für ASP.NET und Anleitungen zum Auswählen von Plattformen und Zugriff auf Methoden, die für Ihr Szenario geeignet sind.
...
- [Ansätze zur Entity Framework-Entwicklung](https://msdn.microsoft.com/en-us/library/ms178359.aspx#dbfmfcf) (MSDN). Leitfaden zum Auswählen einer Entity Framework Entwicklungsansatz Database First Model First oder Code First.
...
- [Entity Framework - Code zunächst zu einer vorhandenen Datenbank](https://msdn.microsoft.com/en-us/data/jj200620). MSDN. Video und exemplarische Vorgehensweise, die zeigt, wie Sie mithilfe von Code First mit einer vorhandenen Datenbank.
- [Data Developer Center - Entity Framework](https://msdn.microsoft.com/data/ef). MSDN. Anleitung zur Dokumentation zu Entity Framework, die vom Entity Framework-Team verwaltete und erstellt wurden, finden Sie unter der [Einstieg in die](https://msdn.microsoft.com/en-us/data/ee712907) Link.
...
- [Übersicht über die Bereitstellung für Visual Studio und ASP.NET Web](https://msdn.microsoft.com/en-us/library/dd394698.aspx#dbdeployment). Finden Sie unter der **Konfigurieren von Datenbank-Bereitstellung in Visual Studio** Abschnitt eine Erläuterung, wie Code First-Migrationen in Visual Studio Web Bereitstellungsfeatures integriert wird.
- [Data Developer Center - Code First-Migrationen](https://msdn.microsoft.com/en-us/data/jj591621) (MSDN). Das Entity Framework-Team Migrationen-Dokumentation.
...
- [Data Developer Center - Laden von verknüpften Entitäten](https://msdn.microsoft.com/en-us/data/jj574232) (MSDN). Das Entity Framework-Team-Dokumentation zum Laden von Daten beziehen.
...
- [Überlegungen zur Leistung für Entity Framework 5](https://msdn.microsoft.com/en-us/data/hh949853) (MSDN).
- [Überlegungen zur Leistung (Entity Framework)](https://msdn.microsoft.com/en-us/library/cc853327) (MSDN).
...
- [Data Developer Center – vollständige Parallelität Muster](https://msdn.microsoft.com/en-us/data/jj592904) (MSDN). Das Entity Framework-Team Concurrency-Dokumentation.
...
- [Daten für Webserversteuerelemente](https://msdn.microsoft.com/en-us/library/ms247258.aspx) (MSDN).
...
- [Datengebundene Webserversteuerelemente](https://msdn.microsoft.com/en-us/library/ms228214.aspx) (MSDN).
- [Ausdrücke Übersicht über Datenbindung](https://msdn.microsoft.com/en-us/library/ms178366.aspx) (MSDN). Diese Seite nur deckt **Eval** und **binden**; wurde nicht onlinesignierung aktualisiert **Element** und **BindItem**.

<a id="sqlserver"></a>

## <a name="working-with-sql-server-databases"></a>Arbeiten mit SQL Server-Datenbanken

- [SQL Server-Datenbankfunktionen](https://msdn.microsoft.com/en-us/library/hh230827.aspx) (MSDN). Eine allgemeine Einführung in einer Vielzahl von SQL Server-Themen werden soll finden Sie in den Einträgen unter diesem im Inhaltsverzeichnis angezeigt.
- [SQL Server-Editionen](https://msdn.microsoft.com/en-us/library/ms178359.aspx#sqlserver) (MSDN). Eine Zusammenfassung der verfügbaren SQL Server-Editionen, mit Links zu weiteren Informationen über die einzelnen Aktionen.)
- [SQL Server-Verbindungszeichenfolgen für ASP.NET-Webanwendungen](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN).
- [Mithilfe von SQL Server Compact für ASP.NET-Webanwendungen](https://msdn.microsoft.com/en-us/library/ms247257.aspx) (MSDN).
...
- [SQLServer Express 2012 LocalDB](https://msdn.microsoft.com/en-us/library/hh510202(v=sql.110).aspx) (MSDN). Die offizielle auf "LocalDB" MSDN-Einführung.
- [SQL Server-Verbindungszeichenfolgen für ASP.NET-Webanwendungen](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN).
- [Vorgehensweise: Aktualisieren auf "LocalDB"](https://msdn.microsoft.com/en-us/library/hh873188.aspx) (MSDN). So migrieren Sie eine MDF-Datei von einer früheren Version von SQL Server Express auf "LocalDB". Außerdem müssen Sie diesen Prozess durchlaufen, wenn Sie eine der Herunterladen der [Beispieldatenbanken für SQL Server 2012](https://go.microsoft.com/fwlink/?linkid=117483).
...
- [SQL Server-Verbindungszeichenfolgen für ASP.NET-Webanwendungen](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN). Wenn Sie die Einstellung für die AttachDBFileName Verbindungszeichenfolge mit SQL Server Express verwenden, finden Sie insbesondere die Benutzerinstanz Abschnitt auf dieser Seite.
...
- [Kann meine ASP.NET-Webanwendung eine SQL Server Express-Datenbank in einer produktionsumgebung verwenden?](https://msdn.microsoft.com/en-us/library/jj653753.aspx#sql_express_in_production) (MSDN).
...
- [Windows Azure SQL-Datenbank](https://msdn.microsoft.com/en-us/library/windowsazure/ee336279.aspx) (MSDN). Der Knoten der obersten Ebene des Inhaltsverzeichnisses für SQL-Datenbank in MSDN.
...
- [Anwendungsblock für die Behandlung vorübergehender Fehler](https://msdn.microsoft.com/en-us/library/hh680934(v=PandP.50).aspx). Ein Framework, das Sie die vorübergehenden Netzwerkfehlern und Verbindungsfehler zu handhaben, die von der Einschränkung zu kann. In einem NuGet-Paket zur Verfügung: [Enterprise Library 5.0 - vorübergehenden Anwendungsblock zur Behandlung](http://nuget.org/packages/EnterpriseLibrary.WindowsAzure.TransientFaultHandling).
- [Erste Schritte mit SQL-Datenbank und Entity Framework](https://msdn.microsoft.com/en-us/data/jj556244) (MSDN).
- [Windows Azure-Trainingskit](https://www.microsoft.com/en-us/download/details.aspx?id=8396) (Microsoft Download Center). Enthält praktische Übungseinheiten für SQL-Datenbank.
- [Community-Forum für Windows Azure SQL-Datenbank](https://social.msdn.microsoft.com/Forums/en-US/ssdsgetstarted/threads).
- [Verschieben von Windows Azure SQL-Datenbank](https://msdn.microsoft.com/en-us/library/ff803375.aspx) (MSDN). Ein Kapitel eine umfassende End-to-End-Szenario vom Microsoft Patterns and Practices-Team. Beschreibt, warum Sie möglicherweise migrieren möchten, und zum Migrieren von SQL Server mit SQL-Datenbank.
- [Migrieren von SQL Server-Datenbanken zu Windows Azure SQL-Datenbank](https://msdn.microsoft.com/en-us/library/windowsazure/jj156160.aspx) (MSDN).
...
- [Migration von Daten zu Windows Azure SQL-Datenbank: Tools und Techniken](https://msdn.microsoft.com/en-us/library/windowsazure/hh694043.aspx) (MSDN). Enthält Abschnitte, die SQL Server mit SQL-Datenbank vergleichen und bieten eine Anleitung dazu, wann von SQL Server mit SQL-Datenbank zu migrieren.
...
- [Einschränkungen für die SQL Server-Funktionen (Windows Azure SQL-Datenbank)](https://msdn.microsoft.com/en-us/library/windowsazure/ff394115.aspx) (MSDN).
- [Windows Azure-Tabellenspeicher und Windows Azure SQL-Datenbank – Vergleich und Gegenüberstellung](https://msdn.microsoft.com/en-us/library/jj553018.aspx) (MSDN). Für eine Anwendung, die Sie in Windows Azure bereitstellen, kann Windows Azure-Tabellenspeicher eine Alternative zum Windows Azure SQL-Datenbank sein. Dieses Thema hilft Ihnen bei der Entscheidung zwischen diesen alternativen.
- [Windows Azure SQL-Datenbank](https://msdn.microsoft.com/en-us/library/windowsazure/ee336279) (MSDN).
- [Richtlinien und Einschränkungen (Windows Azure SQL-Datenbank)](https://msdn.microsoft.com/en-us/library/windowsazure/ff394102.aspx)

<a id="nosql"></a>

## <a name="working-with-nosql-database-management-systems"></a>Arbeiten mit NoSQL-Datenbank-Managementsysteme

- [Windows Azure-Datendienste](https://www.windowsazure.com/en-us/develop/net/data/) (Microsoft Azure-Website). Finden Sie unter der [Tabellendienst funktionsleitfaden](https://docs.microsoft.com/azure/) und **Big Data** Abschnitt der Seite.
...
- [ASP.NET-Daten Zugriffsoptionen](https://msdn.microsoft.com/en-us/library/ms178359.aspx#linq) (MSDN). Enthält eine Einführung in LINQ.
...
- [Dynamic Data-Projektvorlagen](https://msdn.microsoft.com/en-us/library/jj822927.aspx#dynamicdata) (MSDN). Hinweise dazu, wann Dynamic Data-Projekte verwendet.
...
- [Schützen des Datenzugriffs in ASP.NET](https://msdn.microsoft.com/en-us/library/ms178375.aspx) (MSDN).
- [Sicherheitsüberlegungen (Entity Framework)](https://msdn.microsoft.com/en-us/library/cc716760.aspx) (MSDN).
- [Vorgehensweise: Sichern von Verbindungszeichenfolgen bei Verwendung von Steuerelementen für Datenquellen](https://msdn.microsoft.com/en-us/library/ms178372.aspx) (MSDN).

<a id="optimizingdataaccess"></a>

## <a name="optimizing-data-access-performance"></a>Optimieren der Leistung im Zugriff

- [Übersicht über die Leistung von ASP.NET](https://msdn.microsoft.com/en-us/library/cc668225.aspx) (MSDN).
- [ASP.NET-Caching](https://msdn.microsoft.com/en-us/library/xsbfdd8c.aspx) (MSDN).
- [Verbessern der ASP.NET-Leistung](https://msdn.microsoft.com/en-us/library/ff647787) (MSDN). Eine "Inhalt veraltet" Warnung am oberen Rand dieser Seite jedoch die meisten Informationen noch relevant ist, und es gibt keine vergleichbare aktualisierte Ressource.
- [Verbessern der Leistung von SQL Server](https://msdn.microsoft.com/en-us/library/ff647793) (MSDN). Kommentar als der vorherige Link.
...
- [Zugreifen auf Daten über einen Webdienst](https://msdn.microsoft.com/en-us/library/ms178359.aspx#webservice) (MSDN). Hinweise dazu, wann im Vergleich zu WCF-Web-API verwenden.
...
- [Häufig gestellte Fragen zum Datenzugriff mit ASP.NET](https://msdn.microsoft.com/en-us/library/jj653753.aspx) (MSDN).
- [ASP.NET Web Forms-Lernprogramme - Daten](../web-forms/overview/data-access/index.md). Die meisten diese Lernprogramme sind relativ alte; Lesen Sie unbedingt [ASP.NET Datenzugriffsoptionen](https://msdn.microsoft.com/en-us/library/ms178359.aspx) und [Datenspeicheroptionen (Building Real-World Cloud Apps mit Windows Azure)](../aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/data-storage-options.md) erste, damit Sie nicht zu weit in einen Datenzugriffsmethode erhalten, die nicht von rechts für Ihr Szenario.
...
- [Zugreifen auf Daten in Visual Studio](https://msdn.microsoft.com/en-us/library/wzabh8c4.aspx) (MSDN). Enthält eine Liste der Links, die ähnlich wie auf diese inhaltszuordnung jedoch mit dem Schwerpunkt auf Visual Studio anstelle von ASP.NET.
