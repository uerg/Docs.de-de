...
> <span data-ttu-id="bb7be-166">Die neue `Html.Raw` -Methode können Sie das Rendern von HTML-Markup als Markup und nicht codierte Ausgabe rendern.</span><span class="sxs-lookup"><span data-stu-id="bb7be-166">The new `Html.Raw` method lets you render HTML markup as markup instead of rendering encoded output.</span></span> <span data-ttu-id="bb7be-167">(Standardmäßig codiert ASP.NET Razor Zeichenfolgen vor dem Rendern sie.) Die Syntax lautet:</span><span class="sxs-lookup"><span data-stu-id="bb7be-167">(By default, ASP.NET Razor encodes strings before rendering them.) The syntax is:</span></span>
> 
> `Html.Raw(value)`
> 
...
> 
> - <span data-ttu-id="bb7be-278">Wenn Sie die Kontrolle über den Server-Computer verfügen, installieren Sie auf dem Computer das Update, das beschrieben ist [ein Update ist verfügbar, können Sie bestimmte IIS 7.0 oder IIS 7.5-Handler behandelt Anforderungen, deren URLs nicht mit einem Punkt enden](https://support.microsoft.com/kb/980368).</span><span class="sxs-lookup"><span data-stu-id="bb7be-278">If you have control over the server computer, on the server computer install the update that is described in [A update is available that enables certain IIS 7.0 or IIS 7.5 handlers to handle requests whose URLs do not end with a period](https://support.microsoft.com/kb/980368).</span></span>
...
> <span data-ttu-id="bb7be-333">**Dieses Problem zu umgehen**</span><span class="sxs-lookup"><span data-stu-id="bb7be-333">**Workaround**</span></span>  
...
#### <a name="issue-the-foreign-key-dialog-box-doesnt-allow-entering-a-primary-key"></a><span data-ttu-id="bb7be-389">Problem: Das Dialogfeld "Foreign Key" lässt keine zu einen primären Schlüssel eingeben</span><span class="sxs-lookup"><span data-stu-id="bb7be-389">Issue: The "Foreign Key" dialog box doesn't allow entering a primary key</span></span>

> <span data-ttu-id="bb7be-390">Die **Fremdschlüssel** Dialogfeld lässt nicht zu der Primärschlüsselname geben an der Primärschlüsseltabelle.</span><span class="sxs-lookup"><span data-stu-id="bb7be-390">The **Foreign Key** dialog box does not allow you to enter the primary key name from the primary key table.</span></span>
> 
> <span data-ttu-id="bb7be-391">**Dieses Problem zu umgehen**</span><span class="sxs-lookup"><span data-stu-id="bb7be-391">**Workaround**</span></span>  
> <span data-ttu-id="bb7be-392">Dies ist beabsichtigt.</span><span class="sxs-lookup"><span data-stu-id="bb7be-392">This is intentional.</span></span> <span data-ttu-id="bb7be-393">Sie müssen nicht den Namen des primären Schlüssels auf der Primärschlüsseltabelle eingeben.</span><span class="sxs-lookup"><span data-stu-id="bb7be-393">You do not need to enter the name of the primary key from the primary key table.</span></span>


#### <a name="issue-the-relationships-button-is-disabled"></a><span data-ttu-id="bb7be-394">Problem: Die Schaltfläche "Beziehungen" ist deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="bb7be-394">Issue: The "Relationships" button is disabled</span></span>

> <span data-ttu-id="bb7be-395">Die **Beziehungen** Schaltfläche der **Tabelle** Registerkarte die **Datenbanken** Arbeitsbereich ist für SQL Server Compact-Datenbanken deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="bb7be-395">The **Relationships** button under the **Table** tab in the **Databases** workspace is disabled for SQL Server Compact databases.</span></span>
> 
> <span data-ttu-id="bb7be-396">**Dieses Problem zu umgehen**</span><span class="sxs-lookup"><span data-stu-id="bb7be-396">**Workaround**</span></span>  
> <span data-ttu-id="bb7be-397">Keine</span><span class="sxs-lookup"><span data-stu-id="bb7be-397">None.</span></span> <span data-ttu-id="bb7be-398">SQL Server Compact unterstützt keine Beziehungen zwischen Tabellen.</span><span class="sxs-lookup"><span data-stu-id="bb7be-398">SQL Server Compact does not support relationships between tables.</span></span>


#### <a name="issue-parameterized-sql-queries-throw-exceptions"></a><span data-ttu-id="bb7be-399">Problem: Parametrisierte SQL-Abfragen Auslösen von Ausnahmen</span><span class="sxs-lookup"><span data-stu-id="bb7be-399">Issue: Parameterized SQL queries throw exceptions</span></span>

> <span data-ttu-id="bb7be-400">In SQL Server Compact 4.0, wenn Sie einen Datentyp nicht wie z. B. angeben `SqlDbType` oder `DbType` für Parameter in parametrisierten Abfragen, wird eine Ausnahme ausgelöst, wenn die Abfrage ausgeführt wird.</span><span class="sxs-lookup"><span data-stu-id="bb7be-400">In SQL Server Compact 4.0, if you do not specify a data type such as `SqlDbType` or `DbType` for parameters in parameterized queries, an exception is thrown when the query runs.</span></span>
> 
> <span data-ttu-id="bb7be-401">**Dieses Problem zu umgehen**</span><span class="sxs-lookup"><span data-stu-id="bb7be-401">**Workaround**</span></span>  
> <span data-ttu-id="bb7be-402">Legen Sie den Datentyp für Parameter explizit z.B. `SqlDbType` oder `DbType`.</span><span class="sxs-lookup"><span data-stu-id="bb7be-402">Explicitly set the data type for parameters such as `SqlDbType` or `DbType`.</span></span> <span data-ttu-id="bb7be-403">Dies ist besonders wichtig, im Fall von BLOB-Datentypen (`image` und `ntext`).</span><span class="sxs-lookup"><span data-stu-id="bb7be-403">This is critical in the case of BLOB data types (`image` and `ntext`).</span></span> <span data-ttu-id="bb7be-404">Verwenden Sie Code wie folgt:</span><span class="sxs-lookup"><span data-stu-id="bb7be-404">Use code like the following:</span></span>
> 
...
## <a name="for-more-information"></a><span data-ttu-id="bb7be-405">Weitere Informationen</span><span class="sxs-lookup"><span data-stu-id="bb7be-405">For More Information</span></span>

<span data-ttu-id="bb7be-406">Weitere Informationen zu WebMatrix Beta 3 finden Sie unter den folgenden Websites:</span><span class="sxs-lookup"><span data-stu-id="bb7be-406">For more information about WebMatrix Beta 3, see the following websites:</span></span>

- [<span data-ttu-id="bb7be-407">IIS.net</span><span class="sxs-lookup"><span data-stu-id="bb7be-407">IIS.net</span></span>](http://iis.net/)
- [<span data-ttu-id="bb7be-408">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="bb7be-408">ASP.NET</span></span>](https://asp.net/webmatrix)
- [<span data-ttu-id="bb7be-409">Microsoft.com/web</span><span class="sxs-lookup"><span data-stu-id="bb7be-409">Microsoft.com/web</span></span>](https://www.microsoft.com/web)

* * *

<span data-ttu-id="bb7be-410">© 2010 Microsoft Corporation.</span><span class="sxs-lookup"><span data-stu-id="bb7be-410">© 2010 Microsoft Corporation.</span></span> <span data-ttu-id="bb7be-411">Alle Rechte vorbehalten.</span><span class="sxs-lookup"><span data-stu-id="bb7be-411">All Rights Reserved.</span></span> <span data-ttu-id="bb7be-412">[Nutzungsbedingungen](https://msdn.microsoft.cos/cc300389.aspx).</span><span class="sxs-lookup"><span data-stu-id="bb7be-412">[Terms of Use](https://msdn.microsoft.cos/cc300389.aspx).</span></span>
