...
3. Wählen Sie die `GetHtml` Methode aus der Liste und drücken Sie dann die EINGABETASTE. IntelliSense füllt automatisch die Methode ab. (Wie mit einer beliebigen Methode in c# werden soll, das Sie hinzufügen müssen `()` Zeichen nach der Methode.)  
   Den vollständigen Code für die `GetHtml` Methode sieht wie im folgenden Beispiel aus:  
