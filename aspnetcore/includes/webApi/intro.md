...
* Der Client ist das Medium, das die Web-API nutzt (mobile App, Browser usw.). In diesem Tutorial wird kein Client erstellt. [Postman](https://www.getpostman.com/) oder [curl](https://curl.haxx.se/docs/manpage.html) wird als Client verwendet, um die App zu testen.

* Ein *Modell* ist ein Objekt, das die Daten in der App darstellt. In diesem Fall ist das einzige Modell ein To-do-Element. Modelle werden als C#-Klassen dargestellt, die auch als **P**lain **O**ld **C**LR **O**bjects (POCOs) bezeichnet werden.
