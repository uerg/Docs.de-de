...
* Verweisen auf Projekte `Microsoft.AspNetCore.All` oder `Microsoft.AspNetCore.App` Pakete verweisen müssen `Microsoft.Net.Sdk.Web` Sdk.
* Verweisen auf Pakete oder Projekte, die transitiv verweisen Projekte `Microsoft.AspNetCore.All` oder `Microsoft.AspNetCore.App`:
  * Verweisen muss die `Microsoft.Net.Sdk.Web` Sdk und,
  * Muss auf das gleichen freigegebenen Laufzeitpaket verweisen. Wenn LibraryA verweist z. B. `Microsoft.AspNetCore.App`, müssen alle Projekte, die darauf verweisen verweisen `Microsoft.AspNetCore.App`.
* Ausführbaren Projekten:
  * Projekte, die apps, die mit gestartet werden, enthalten sind `dotnet run`.
  * Sind Anwendungen, die ausgeführt und Testprojekte für Anwendungen.
  * Ausführbare Projekten müssen nicht die Version angeben. Das Sdk gibt die Version implizit über `<PackageReference Include="Microsoft.AspNetCore.App" />`.
* Referenzierten Projekten ist:
  * Projekte, die den Einstiegspunkt sind und
  * Die Projektverweise `Microsoft.AspNetCore.All` oder `Microsoft.AspNetCore.App`.
  * Referenzierte Projekten müssen eine Version des Pakets angeben.
...
ASP.NET Core 2.1 bietet [ASP.NET Core Identity](xref:security/authentication/identity) als eine [Razor-Klassenbibliothek](xref:razor-pages/ui-class). Wenn Sie keine erhebliche vorgenommen haben Änderungen an der Vorlage 2.0 generierter Identitätscode, den folgenden Upgrade Ansatz in Betracht ziehen:
