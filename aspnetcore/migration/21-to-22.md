...
Übernehmen der [: prozessinternes Hostingmodell für IIS](xref:fundamentals/servers/aspnet-core-module#in-process-hosting-model), Hinzufügen der `<AspNetCoreHostingModel>` Eigenschaft mit einem Wert von `InProcess` auf eine `<PropertyGroup>` in der Projektdatei:

```xml
<AspNetCoreHostingModel>InProcess</AspNetCoreHostingModel>
```

Weitere Informationen finden Sie unter <xref:host-and-deploy/aspnet-core-module#hosting-model>.
