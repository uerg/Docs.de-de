...
ASP.NET Core unterstützt das Softwareentwurfsmuster Abhängigkeitsinjektion. Damit kann eine [Umkehrung der Steuerung](https://deviq.com/inversion-of-control/) (Inversion of Control, IoC) zwischen Klassen und ihren Abhängigkeiten erreicht werden.
...
> [!NOTE]
> Jede Erweiterungsmethode vom Typ `services.Add<ServiceName>` fügt Dienste hinzu (und konfiguriert diese möglicherweise). So fügt beispielsweise `services.AddMvc()` die erforderlichen Dienste Razor Pages und MVC hinzu. Es wird empfohlen, dass Apps dieser Konvention folgen. Platzieren Sie Erweiterungsmethoden im Namespace [Microsoft.Extensions.DependencyInjection](/dotnet/api/microsoft.extensions.dependencyinjection), um Gruppen von Dienstregistrierungen zu kapseln.
...
Wenn eine Klasse zu viele eingefügte Abhängigkeiten zu haben scheint, ist dies im Allgemeinen ein Zeichen dafür, dass die Klasse zu viele Aufgaben hat und gegen das [Single-Responsibility-Prinzip (SRP)](https://deviq.com/single-responsibility-principle/) (Prinzip der eindeutigen Verantwortlichkeit) verstößt. Versuchen Sie, die Klasse umzugestalten, indem Sie einige ihrer Aufgaben in eine neue Klasse verschieben. Beachten Sie, dass der Fokus der Razor Pages-Seitenmodellklassen und MVC-Controllerklassen auf der Benutzeroberfläche liegt. Geschäftsregeln und Implementierungsdetails für den Datenzugriff sollten in Klassen aufbewahrt werden gemäß dem Prinzip [Separation of Concerns](https://deviq.com/separation-of-concerns/) (Trennung der Zuständigkeiten).
...
Der integrierte Dienstcontainer dient dazu, die grundlegenden Anforderungen des Frameworks und der meisten darin integrierten Consumeranwendungen zu erfüllen. Entwickler können den integrierten Container jedoch durch ihren bevorzugten Container ersetzen. Die `Startup.ConfigureServices`-Methode gibt normalerweise `void`zurück. Wenn die Signatur der Methode so geändert wird, dass [IServiceProvider](/dotnet/api/system.iserviceprovider) zurückgegeben wird, kann ein anderer Container konfiguriert und zurückgegeben werden. Für .NET sind viele IoC-Container verfügbar. Im folgenden Beispiel wird der [Autofac](https://autofac.org/)-Container verwendet:

1. Installieren Sie das entsprechende Containerpaket bzw. die entsprechenden Containerpakete:

    * [Autofac](https://www.nuget.org/packages/Autofac/)
    * [Autofac.Extensions.DependencyInjection](https://www.nuget.org/packages/Autofac.Extensions.DependencyInjection/)

2. Konfigurieren Sie den Container in `Startup.ConfigureServices`, und geben Sie einen `IServiceProvider`-Wert zurück:
...
3. Konfigurieren Sie Autofac in `DefaultModule`:
...
* Vermeiden Sie die Verwendung des Dienstlocatormusters (z. B. [IServiceProvider.GetService](/dotnet/api/system.iserviceprovider.getservice)).
...
Dependency Injection stellt eine *Alternative* zu statischen bzw. globalen Objektzugriffsmustern dar. Möglicherweise können Sie die Vorteile der Abhängigkeitsinjektion nicht nutzen, wenn Sie sie mit dem Zugriff auf statische Objekte kombinieren.
