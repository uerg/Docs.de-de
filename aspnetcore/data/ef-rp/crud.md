...
---
# <a name="create-read-update-and-delete---ef-core-with-razor-pages-2-of-8"></a>Erstellen, Lesen, Aktualisieren und Löschen: EF Core mit Razor-Seiten (2 von 8)
...
Hinweis: Zur Minimierung der Komplexität und damit EF Core im Fokus dieser Tutorials bleibt, wird in den Seitenmodellen auf den Razor-Seiten EF Core-Code verwendet. Einige Entwickler verwenden eine Dienstschicht oder ein Repositorymuster für die Erstellung einer Abstraktionsschicht zwischen der Benutzeroberfläche (Razor-Seiten) und der Datenzugriffsschicht.

In diesem Tutorial werden die Razor-Seiten „Create“ (Erstellen), „Edit“ (Bearbeiten), „Delete“ (Löschen) und „Details“ im Ordner *Student* geändert.
...
In einem Großteil des Codes kann [FindAsync](https://docs.microsoft.com/dotnet/api/microsoft.entityframeworkcore.dbcontext.findasync?view=efcore-2.0#Microsoft_EntityFrameworkCore_DbContext_FindAsync_System_Type_System_Object___) anstelle von `FirstOrDefaultAsync` oder `SingleOrDefaultAsync` verwendet werden. 
...
Aktualisieren Sie die Razor-Seiten „Edit“ (Bearbeiten), „Details“ und „Delete“ (Löschen) so, dass die Routenvorlage `"{id:int}"` verwendet wird. Ändern Sie die „page“-Direktive für jede dieser Seiten aus `@page` in `@page "{id:int}"`.
...
Überprüfen Sie den [TryUpdateModelAsync](https://docs.microsoft.com/ dotnet/api/microsoft.aspnetcore.mvc.controllerbase.tryupdatemodelasync?view=aspnetcore-2.0#Microsoft_AspNetCore_Mvc_ControllerBase_TryUpdateModelAsync_System_Object_System_Type_System_String_)-Code:

[!code-csharp[Main](intro/samples/cu/Pages/Students/Create.cshtml.cs?name=snippet_TryUpdateModelAsync)]

Im vorangehenden Code versucht `TryUpdateModelAsync<Student>`, das Objekt `emptyStudent` mithilfe der bereitgestellten Formularwerte aus der Eigenschaft [PageContext](https://docs.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.razorpages.pagemodel.pagecontext?view=aspnetcore-2.0#Microsoft_AspNetCore_Mvc_RazorPages_PageModel_PageContext) im [PageModel](https://docs.microsoft.com/dotnet/api/microsoft.aspnetcore.mvc.razorpages.pagemodel?view=aspnetcore-2.0) zu aktualisieren. `TryUpdateModelAsync` aktualisiert nur die aufgeführten Eigenschaften (`s => s.FirstMidName, s => s.LastName, s => s.EnrollmentDate`).
...
Auf Razor-Seiten stellt die abgeleitete `PageModel`-Klasse das Ansichtsmodell dar. 

## <a name="update-the-edit-page"></a>Aktualisieren der Seite „Bearbeiten“

Aktualisieren Sie das Seitenmodell für die Seite „Bearbeiten“:
...
* `FirstOrDefaultAsync` wurde durch [FindAsync](https://docs.microsoft.com/dotnet/api/microsoft.entityframeworkcore.dbset-1.findasync?view=efcore-2.0) ersetzt. `FindAsync` eignet sich für die Auswahl einer Entität über den Primärschlüssel. Weitere Informationen hierzu finden Sie unter [FindAsync](#FindAsync).
...
### <a name="update-the-delete-razor-page"></a>Aktualisieren der Razor-Seite „Löschen“

Fügen Sie folgende hervorgehobene Fehlermeldung zur Razor-Seite „Löschen“ hinzu.
...
Überprüfen Sie, ob die Razor-Seite die richtige `@page`-Anweisung enthält. Die Razor-Seite „Student/Home“ sollte beispielsweise **keine** Routenvorlage enthalten:

```cshtml
@page "{id:int}"
```

Die `@page`-Anweisung muss auf jeder Razor-Seite vorhanden sein.
